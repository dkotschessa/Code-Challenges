...
A dictionary comprehension is like a list comprehension, but it
 constructs a dict instead of a list. They are convenient to quickly
  operate on each (key, value) pair of a dict. And often in one line of
   code, maybe two after checking PEP8 ;)
...
def filter_bites(bites=bites, bites_done=exclude_bites):
    ...
    >>> filter_bites({1: 'bob', 2: 'julian', 3: 'tim'}, {2, 3})
    {1: 'bob'}
    """
    excluded = {x: y for x, y in bites.items() if x not in bites_done}
    return excluded
